...
class AverageTests: XCTestCase {
    ...
    func test_average_empty() {
        ...
        XCTAssert(data.average(by: \.int).isNaN)
        XCTAssert(data.average(by: \.float).isNaN)
        XCTAssert(data.average(by: \.double).isNaN)
    }
    ...
    func test_average_values() {
        ...
        XCTAssert(areEqual(data.average(by: \.int), 14.166))
        XCTAssert(areEqual(data.average(by: \.float), 9.4))
        XCTAssert(areEqual(data.average(by: \.double), 12.845))
    }
}
