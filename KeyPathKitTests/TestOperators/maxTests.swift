...
class MaxTests: XCTestCase {
    ...
    func test_max_empty() {
        ...
        XCTAssertNil(data.max(\.int))
        XCTAssertNil(data.max(\.double))
    }
    ...
    func test_max_values() {
        ...
        XCTAssertEqual(data.max(\.int), TestData(int: 70, double: 8.0))
        XCTAssertEqual(data.max(\.double), TestData(int: -3, double: 65.0))
    }
}
